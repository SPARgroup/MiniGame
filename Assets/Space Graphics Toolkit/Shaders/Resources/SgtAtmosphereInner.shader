...
Shader "Hidden/SgtAtmosphereInner"
{
	Properties
	{
		...
		_ScatteringPower("Scattering Power", Float) = 3
		...
		Pass
		{
			...
			CGPROGRAM
	...
	#endif
	#if SGT_C
					float4 texcoord4 : TEXCOORD4; // camera to vertex/pixel
	#endif
#endif
				...
				void Vert(a2v i, out v2f o)
				{
					...
					float3 near = mul(_WorldToLocal, float4(_WorldSpaceCameraPos, 1.0f));
#if SGT_A
	...
	#endif
	#if SGT_C
					o.texcoord4.w   = pow(horizon, _ScatteringPower);
					o.texcoord4.xyz = _WorldSpaceCameraPos - wPos.xyz;
	...
	#endif
	#if SGT_C
					i.texcoord4.xyz = normalize(i.texcoord4.xyz);

					float4 scattering = MieRayleighPhase(dot(i.texcoord4.xyz, normalize(i.texcoord2.xyz)), _Mie, _Rayleigh) * light1;
		#if LIGHT_2
					scattering += MieRayleighPhase(dot(i.texcoord4.xyz, normalize(i.texcoord3.xyz)), _Mie, _Rayleigh) * light2;
		#endif
		#if SHADOW_1 || SHADOW_2
					scattering *= shadowColor.w;
		#endif
		#if SGT_D
					lighting += saturate(scattering * i.texcoord4.w) * saturate(1.0f - (o.color + lighting)); // Make the scattering fill the remaining color
		#else
					lighting += scattering * i.texcoord4.w; // Allow overflow
		#endif
	#endif
					o.color += lighting;
